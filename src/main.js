...
function saveRun() {
  ...
  cave.run = { x: player.x, y: player.y, z: player.z, yaw: player.yaw, battery: player.battery, breath: player.breath, hurt: player.hurt, sticks: player.sticks, rope: player.rope, cells: player.cells, kit: player.kit, suit: player.suit, cold: player.cold, frames: framesLeft,
...
function useRope() {
  ...
  if (!v) {
    ...
    const need = Math.max(1, Math.ceil(se.path.len / ROPE_COIL_M));                               // a coil has the same usable length in a sump or down a pitch
  ...
  const need = Math.max(1, Math.ceil((v.top - v.y) / ROPE_COIL_M));                              // a coil is about twelve metres
  ...
  const r = { x: v.x, z: v.z, top: v.top, bottom: v.y, coils: need, mesh: new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, v.top - v.y + 0.3, 5), ropeMat) };
...
function restoreRopes() {
  for (const e of cave.ropes || []) {
    ...
    r.mesh.position.set(e.x, (e.top + e.bottom) / 2 - 0.1, e.z); scene.add(r.mesh); dressRope(r); ropes.push(r);
  }
}
restoreRopes();
...
// pulls you along it toward the bank you face when you grab it — faster than swimming, and it still works when the silt is up. Lines stay laid.
...
let hauled = false, haulT = 0, padUseHeld = false, padUseAt = null, lineRestoreT = 0, haulDirection = null;
...
function updateHaul(dt) {
  ...
  if (!holding || !player.swim) { if (!holding) haulT = 0; haulDirection = null; return; }
  const nl = nearestLine(1.7); if (!nl) return;
  hauled = true; haulT += dt;
  if (!haulDirection || haulDirection.line !== nl.L) {
    const ahead = linePoint(nl.L, Math.min(nl.L.len, nl.s + .5)), behind = linePoint(nl.L, Math.max(0, nl.s - .5));
    camera.getWorldDirection(viewDir);
    const intent = (ahead.x - behind.x) * viewDir.x + (ahead.y - behind.y) * viewDir.y + (ahead.z - behind.z) * viewDir.z;
    haulDirection = { line: nl.L, sign: Math.abs(intent) > .05 ? Math.sign(intent) : nl.s < nl.L.len / 2 ? -1 : 1 };
  }
  const s2 = clamp(nl.s + haulDirection.sign * 3.0 * dt, 0, nl.L.len), q0 = linePoint(nl.L, nl.s), q1 = linePoint(nl.L, s2), k = Math.min(1, dt * 6);
  ...
  teach('haul', 'the line: look toward the bank you want, hold E and pull yourself along it. Release to change direction. faster than swimming, and it knows the way out even when you cannot see it');
}
...
function updateStones(dt) {
  for (let i = stones.length - 1; i >= 0; i--) {
    ...
    for (let k = 0; k < 3 && !done; k++) {
      ...
      if (Number.isFinite(wl) && g.y < wl) { vfx.emit('splash',g.x,visualWater(g.x,g.y,g.z),g.z,6); sfx.play('splash_small', { x: g.x, y: wl, z: g.z, vol: 0.5, wet: 0.6 }); done = true; }
    }
...
function placeCamp(p) {
  ...
  for (let k = 0; k < 3; k++) { const q = spot(p.rx * 0.5); if (!q) continue; const m = assets.model('sleepingbag'); m.position.set(q.x,q.y,q.z); m.rotation.y=R()*6.28; scene.add(m); campDecor.push(m); }
  const st = spot(p.rx * 0.3); if (st) { const m=assets.model('stove'); m.position.set(st.x,st.y,st.z); scene.add(m); campDecor.push(m); }
...
function placeStraws(p) {
  let sd = p.seed * 233280 | 0; const R = () => (sd = (sd * 9301 + 49297) % 233280) / 233280;
  const cl = { x: p.x, z: p.z, floor: p.floor, tips: [] };
  for (let i = 0; i < p.n; i++) {
    const a = R() * Math.PI * 2, d = Math.sqrt(R()) * p.r, x = p.x + Math.sin(a) * d, z = p.z + Math.cos(a) * d;
  ...
  if (cl.tips.length) strawClusters.push(cl);
}
let strawDripT = 0;
...
function placeSinkhole(p) {
  ...
  if (!p.window && cave.deaths.length >= 6 && !ropes.some(r => r.rescue)) {   // someone up there has counted: a rope comes down the hole
    ...
    r.mesh.position.set(p.x, (p.y + p.floor) / 2 - 0.15, p.z); scene.add(r.mesh); dressRope(r); ropes.push(r);
...
function clearInputs() {
  ...
  glowHeldAt = null; chargeT = 0; beamNarrow = false; eHeldAt = 0; padUseHeld = false; padUseAt = null; hauled = false; haulDirection = null;
}
...
function pollGamepad(dt) {
  ...
  if (!gp) {
    if (padSeen) { for (const k of merged) keys[k] = !!keyboard[k]; padSeen = false; padCharge = padFocus = padUseHeld = false; padUseAt = null; hauled = false; haulDirection = null; glowHeldAt = null; for (const k in padPrev) delete padPrev[k]; if (toolsOpen && wheelSource === 'gamepad') closeTools(); }
    return;
  }
  padSeen = true;
...
function rumble(strong, weak, ms) {
  ...
  for (const g of pads) if (g && g.connected && g.vibrationActuator && g.vibrationActuator.playEffect) { try { Promise.resolve(g.vibrationActuator.playEffect('dual-rumble', { duration: ms, strongMagnitude: strong, weakMagnitude: weak })).catch(() => {}); } catch (e) {} break; }
}
...
function updateContext() {
  ...
  if (canAct() && !typing && !toolsOpen && !notebookOpen) {
    ...
    else if (player.swim && nearestLine(1.7)) text = padSeen ? 'Face the bank · hold D-pad up to haul' : 'Face the bank · hold E to haul';
    else if (lines.some(L => lineEndsNear(L, 2.5))) text = padSeen ? 'Hold D-pad up · reel in line' : 'Hold E · reel in line';
...
function localMesh(mesh, hit, radius) {
  const pos = mesh.geometry.attributes.position.array, center = mesh.worldToLocal(hit.clone()), r = radius * 1.6 + 0.4, out = [];
  for (let i = 0; i < pos.length; i += 9) {
    // Include every triangle whose bounds touch the decal region, even when its first vertex is outside.
    let touches = true;
    for (let a = 0; a < 3; a++) {
      const lo = Math.min(pos[i+a],pos[i+3+a],pos[i+6+a]), hi = Math.max(pos[i+a],pos[i+3+a],pos[i+6+a]);
      const c = a === 0 ? center.x : a === 1 ? center.y : center.z;
      if (lo > c+r || hi < c-r) { touches=false;break; }
    }
    if (touches) for (let k = 0; k < 9; k++) out.push(pos[i + k]);
  }
...
function updateDrips(dt) {
  if (!dripInst) return;
  for (let i = dripDrops.length - 1; i >= 0; i--) { const d = dripDrops[i]; d.v += 9.8 * dt; d.y -= d.v * dt; if (d.y <= d.landY) { const wl=visualWater(d.x,d.landY,d.z); if(Number.isFinite(wl)&&Math.abs(wl-d.landY)<.4) vfx.ripple(d.x,wl,d.z); dripDrops.splice(i, 1); } }
...
function stealTorch() {
  ...
  carried = { start: {x:player.x,y:player.y,z:player.z}, path, t: 0, dur: 2.2 * path.length, stepT: 0 };
...
function updateCarried(dt) {
  ...
  const a = seg === 0 ? carried.start : carried.path[seg - 1], b = carried.path[seg];
  ...
  for (const L of loose) {
    ...
    if (L.state === 'hanging') {
      ...
      if (L.mesh.position.y <= L.rest) {
        L.mesh.position.y = L.rest; L.state = 'down'; vfx.emit('dust',L.x,L.rest,L.z,40);vfx.emit('chips',L.x,L.rest,L.z,20); rumble(0.9, 0.6, 500);
...
function takePhoto() {
  ...
  framesLeft--; photoReq = settings.reduced ? 1 : 2;                                          // the flash lights the next frame; the one after it is the print
...
function afterRenderPhoto(dt) {
  if (photoReq) {
    ...
    if (!photoReq) {
      ...
      for (const data of photoImgs.keys()) if (!cave.photos.some(ph => ph.data === data)) photoImgs.delete(data);
      saveRun();
      if (settings.reduced) { returnLight(flashLight); flashLight = null; flashHold = 0; presentation.render(scene, camera, gameClock * .001); }
    }
  }
  if (flashHold > 0) {
    flashHold -= dt;
    if (flashHold <= 0) {                                              // the burst is gone; what stays is the burn on your eyes
      ...
      const fl = $('flash'); fl.style.transition = 'none'; fl.style.opacity = settings.reduced ? 0 : 0.8; void fl.offsetWidth; fl.style.transition = 'opacity 0.7s ease-out'; fl.style.opacity = 0;
...
// warm the shaders now, not the first time a lake or a loose block comes into view (a compile can cost a quarter second)
{
  ...
  const warm = [new THREE.Mesh(g, waterMat), new THREE.Mesh(looseGeo, looseMat), new THREE.Mesh(olmGeo, olmMat), new THREE.Mesh(pageGeo, pageMat), new THREE.Mesh(packGeo, packMat), new THREE.Mesh(torchGeo, torchMat), new THREE.Mesh(stickGeo, stickMat),
                new THREE.Mesh(g, curtainMat), new THREE.Mesh(g, mistMat), new THREE.Mesh(g, exitGrassMat), new THREE.Mesh(g, exitTreeMat), new THREE.Mesh(bagGeo, bagMat), new THREE.Mesh(stoveGeo, stoveMat), new THREE.Mesh(g, lineMat), new THREE.Mesh(g, strawInst.mesh.material), new THREE.Mesh(stoneGeo, stoneMat), new THREE.Mesh(g, wormMat), new THREE.Mesh(g, bubbleMat),
...
function stepFrame(dt) {
  ...
  if (running && player.alive && !player.out) { updatePlayer(dt); runTime += dt; saveT += dt; if (saveT > 5) { saveT = 0; saveRun(); }
    if (runTime > 40 && runTime < 41) teach('survey-tools', 'Tab opens your survey. Hold Q for chalk, whistle, rest and a stone. G drops a glowstick; hold to throw'); }
  lap('player');
  ...
  updateThrown(dt);
  updateStones(dt);
  updateDrips(dt); updateStraws(dt);
  updateDrops(dt);
  updateBubbles(dt);
  whistleT -= dt;
  updateFollower(dt);
  ...
  beforeRenderPhoto(); presentation.render(scene, camera, gameClock*.001); afterRenderPhoto(dt); lap('render');
